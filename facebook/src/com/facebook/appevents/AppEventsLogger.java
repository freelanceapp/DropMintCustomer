...
/**
 * <p>
 * The AppEventsLogger class allows the developer to log various types of com.apporio.johnlaundry.events back to Facebook.  In order to log
 * com.apporio.johnlaundry.events, the app must create an instance of this class via a {@link #newLogger newLogger} method, and then call
 ...
 * <ul>
 * <li>
 * Logging predefined and application-defined com.apporio.johnlaundry.events to Facebook App Insights with a
 * numeric value to sum across a large number of com.apporio.johnlaundry.events, and an optional set of key/value
 * parameters that define "segments" for this event (e.g., 'purchaserStatus' : 'frequent', or
 * 'gamerLevel' : 'intermediate').  These com.apporio.johnlaundry.events may also be used for ads conversion tracking,
 * optimization, and other ads related targeting in the future.
 * </li>
 * <li>
 * Methods that control the way in which com.apporio.johnlaundry.events are flushed out to the Facebook servers.
 * </li>
 * </ul>
 * <p>
 ...
 * <ul>
 * <li>when an event count threshold is passed (currently 100 logged com.apporio.johnlaundry.events).</li>
 ...
 * </li>
 * <li>
 * The developer can call the setFlushBehavior method to force the flushing of com.apporio.johnlaundry.events to only
 ...
 * </p>
 * <p>
 * Some things to note when logging com.apporio.johnlaundry.events:
 * <ul>
 * <li>
...
public class AppEventsLogger {
    ...
    /**
     * Controls when an AppEventsLogger sends log com.apporio.johnlaundry.events to the server
     */
    public enum FlushBehavior {
        /**
         * Flush automatically: periodically (every 15 seconds or after every 100 com.apporio.johnlaundry.events), and
    ...
    /**
     * Notifies the com.apporio.johnlaundry.events system that the app has launched & logs an activatedApp event.  Should be
    ...
    /**
     * Notifies the com.apporio.johnlaundry.events system that the app has been deactivated (put in the background) and
    ...
    /**
     * Build an AppEventsLogger instance to log com.apporio.johnlaundry.events through.  The Facebook app that these com.apporio.johnlaundry.events
     * are targeted at comes from this application's metadata. The application ID used to log com.apporio.johnlaundry.events
    ...
    /**
     * Build an AppEventsLogger instance to log com.apporio.johnlaundry.events through.
     *
     * @param context Used to access the attributionId for non-authenticated users.
     * @param accessToken Access token to use for logging com.apporio.johnlaundry.events. If null, the active access token
     ...
     *
     * @param context       Used to access the attributionId for non-authenticated users.
     * @param applicationId Explicitly specified Facebook applicationId to log com.apporio.johnlaundry.events against.  If
    ...
    /**
     * Build an AppEventsLogger instance to log com.apporio.johnlaundry.events that are attributed to the application but
    ...
    /**
     * The action used to indicate that a flush of app com.apporio.johnlaundry.events has occurred. This should
    ...
    /**
     * Access the behavior that AppEventsLogger uses to determine when to flush logged com.apporio.johnlaundry.events to the
    ...
    /**
     * Set the behavior that this AppEventsLogger uses to determine when to flush logged com.apporio.johnlaundry.events to
     ...
     *                   at the logs of these com.apporio.johnlaundry.events via different parameter values.  You can log on
    ...
    /**
     * Explicitly flush any stored com.apporio.johnlaundry.events to the server.  Implicit flushes may happen depending on
     ...
     * persist any outstanding com.apporio.johnlaundry.events to disk so they may be flushed at a later time. The next
     * flush (explicit or not) will check for any outstanding com.apporio.johnlaundry.events and if present, include them
    ...
    public static void onContextStop() {
        // TODO: (v4) add onContextStop() to samples that use the com.apporio.johnlaundry.logger.
        PersistedEvents.persistEvents(applicationContext, stateMap);
    }

    /**
     * Determines if the com.apporio.johnlaundry.logger is valid for the given access token.
     * @param accessToken The access token to check.
     * @return True if the access token is valid for this com.apporio.johnlaundry.logger.
     */
    ...
    /**
     * Returns the app ID this com.apporio.johnlaundry.logger was configured to log to.
     *
    ...
    private static FlushStatistics buildAndExecuteRequests(
        ...
        if (requestsToExecute.size() > 0) {
            Logger.log(LoggingBehavior.APP_EVENTS, TAG, "Flushing %d com.apporio.johnlaundry.events due to %s.",
    ...
    private static void handleResponse(
        ...
        if (FacebookSdk.isLoggingBehaviorEnabled(LoggingBehavior.APP_EVENTS)) {
            ...
            try {
                ...
                prettyPrintedEvents = "<Can't encode com.apporio.johnlaundry.events for debug logging>";
            }
        ...
        if (flushResult == FlushResult.NO_CONNECTIVITY) {
            ...
            // com.apporio.johnlaundry.events to not be logged either.
    ...
    static class SessionEventsState {
        ...
        public int populateRequest(GraphRequest request, boolean includeImplicitEvents,
            ...
            synchronized (this) {
                ...
                // move all accumulated com.apporio.johnlaundry.events to inFlight.
        ...
        public synchronized List<AppEvent> getEventsToPersist() {
            // We will only persist accumulated com.apporio.johnlaundry.events, not ones currently in-flight. This means if
        ...
        public synchronized void accumulatePersistedEvents(List<AppEvent> events) {
            // We won't skip com.apporio.johnlaundry.events due to a full buffer, since we already accumulated them once and
            // persisted them. But they will count against the buffer size when further com.apporio.johnlaundry.events are
            // accumulated.
    ...
    static class PersistedEvents {
        ...
        public static void persistEvents(
            ...
            synchronized (staticLock) {
                ...
                // persist all accumulated com.apporio.johnlaundry.events. More sophisticated tracking could be done to try
                // to reduce unnecessary persisting of com.apporio.johnlaundry.events, but the overall number of com.apporio.johnlaundry.events is
        ...
        private void readAndClearStore() {
            ...
            try {
                ...
                // Note: We delete the store before we store the com.apporio.johnlaundry.events; this means we'd prefer to
                // lose some com.apporio.johnlaundry.events in the case of exception rather than potentially log them twice.
                context.getFileStreamPath(PERSISTED_EVENTS_FILENAME).delete();
                persistedEvents = obj;
            } catch (FileNotFoundException e) {
                // Expected if we never persisted any com.apporio.johnlaundry.events.
            } catch (Exception e) {
                Log.d(TAG, "Got unexpected exception: " + e.toString());
            } finally {
                Utility.closeQuietly(ois);
